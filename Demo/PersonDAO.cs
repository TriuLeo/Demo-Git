...
namespace Demo
{
    public class PersonDAO
    {
        ...
        //    }
        //}
        public void Them(string hoTen, string diaChi, string cmnd, string ngaySinh, string table)
        {
            string sqlStr = string.Format($"INSERT INTO {table} (Ten , Diachi , Cmnd, NgaySinh) VALUES ('{0}', '{1}', '{2}', '{3}')"
        ...
        public void Xoa(string cmnd, string table)
        {
            string SQL = string.Format($"DELETE FROM {table} WHERE Cmnd = '{0}'", cmnd);
            DBConnection dbc = new DBConnection();
            dbc.ThucThi(SQL);
        }

        public void Sua(string hoTen, string diaChi, string cmnd, string ngaySinh, string table)
        {
            string SQL = string.Format($"UPDATE {table} SET Ten = '{0}', DiaChi = '{1}', NgaySinh = '{3}' WHERE Cmnd = '{2}'"
                    , hoTen, diaChi, cmnd, ngaySinh);
            DBConnection dbc = new DBConnection();
            dbc.ThucThi(SQL);
        }
